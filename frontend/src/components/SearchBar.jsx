...
import { isValidProductUrl, detectPlatformFromUrl } from '../utils/helpers';
...
export default function SearchBar({ onSearch, loading, error }) {
  ...
  const handleSubmit = useCallback(
    (e) => {
      e.preventDefault();
      if (valid && onSearch) onSearch(url.trim());
    },
    [url, valid, onSearch]
  );
      ...
      <div className="relative flex flex-col gap-2 sm:flex-row">
        <div className="relative flex flex-1 items-center gap-2 rounded-lg border border-white/20 bg-surface-elevated focus-within:border-accent focus-within:ring-1 focus-within:ring-accent">
          <input
            ...
            aria-invalid={!!error}
            aria-describedby={error ? 'search-error' : platform ? 'platform-hint' : undefined}
          ...
          {platform && (
            <span
              id="platform-hint"
              className="mr-2 rounded bg-accent-dim px-2 py-1 font-mono text-xs text-accent"
            >
              {platform}
            </span>
          )}
